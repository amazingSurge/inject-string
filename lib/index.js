...
function openDelim(tag, delims) {
  return emit(`${delims[0]} ${tag} ${delims[1]}`);
}

function closeDelim(tag, delims) {
  return emit(`${delims[0]} end${tag} ${delims[1]}`);
}
...
function split(str, re) {
  return toString(str).split(re).map(seg => emit(seg));
}
...
class InjectString {
  ...
  static inject(src, snippet, opts = {}) {
    ...
    const sections = split(src, regex);
    const contents = stripTags(snippet, regex);
    ...
    const start = emit(sections.shift());
    const end = emit(sections.pop());
    const len = sections.length;

    let inner;
    if (len > 1 && opts.multiple !== false) {
      inner = sections.join(emit(contents));
    } else if (len > 1) {
      inner = emit(sections.shift());
      inner += emit(contents);
